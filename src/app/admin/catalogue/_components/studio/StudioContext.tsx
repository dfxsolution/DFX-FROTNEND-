...
// one "Preview & Export" entry).
//
// Module 30 (post-completion): the "Templates" step was removed at explicit
// user request — the decorative border/frame templates were judged not
// worth the tradeoff of shrinking the product within its canvas, even after
// the compounding-margin bug they were reacting to was independently fixed
// (see SESSION_HANDOFF.md Module 30). 5 entries now, not 6. Save Version /
// Export / Generate PDF / Batch Render in Preview & Export still need SOME
// CanvasDocument to render against — the FULL_BLEED template (product fills
// the entire canvas, no border/overlay) is now auto-selected below instead
// of requiring the user to pick one.
...
interface StudioContextValue {
  ...
  applyEditOperations: (ops: ImageEditOperation[]) => void;
  ...
  // -- Media Library (Step 3 — Watermark logo picker) — fetched once per
  // session, shared across visits, updated in place on upload -------------
  mediaLibrary: ProductImage[];
  mediaLibraryLoading: boolean;
  addToMediaLibrary: (image: ProductImage) => void;
}
...
export const StudioProvider: React.FC<{ productId: string; children: React.ReactNode }> = ({
  ...
  const [furthestUnlockedStep, setFurthestUnlockedStep] = useState(isNewProduct ? 0 : 4);
  ...
  // Same fetch-once rationale as Templates above: WatermarkSection used to
  // fetch this itself in a local useEffect, which re-ran (with a fresh
  // loading flash) every time its parent panel remounted from a step
  // switch — the "unnecessary fetch" this session's Module 30 audit set out
  // to remove (see SESSION_HANDOFF.md Module 30).
  ...
  React.useEffect(() => {
    ...
    catalogueService
      .getMediaLibrary()
      .then((images) => {
        if (!cancelled) setMediaLibrary(images.filter((i) => i.variantType === 'ORIGINAL'));
      })
      .finally(() => {
  ...
  const addToMediaLibrary = useCallback((image: ProductImage) => {
    setMediaLibrary((prev) => [image, ...prev]);
  }, []);
  ...
  /**
   * Same upsert-by-type semantics as applyEditOperation, but for several
   * operations at once — one history entry, one state update, one debounced
   * preview call instead of N round trips. Used by one-click Auto Fit,
   * which chains Remove Empty Space + Auto Center + Auto Scale together
   * (Module 30): applying them one at a time would fire 3 separate preview
   * requests and 3 intermediate renders for what the user experiences as a
   * single action.
   */
  const applyEditOperations = useCallback(
    (ops: ImageEditOperation[]) => {
      ...
      const next = [...editOperations.filter((existing) => !types.has(existing.type)), ...ops];
      setEditOperations(next);
      runEditPreview(next);
    },
    [editOperations, runEditPreview]
  ...
  // Module 30 (post-completion): with the Templates picker removed, this is
  // the only place a template ever gets selected now — automatically, once
  // there's an image to render and the (fetched-once) template list has
  // loaded. FULL_BLEED is a real template like any other (see
  // catalogue_asset_library.py), it just has no border/margin: the product
  // fills its entire canvas. This keeps Save Version/Export/Generate
  // PDF/Batch Render (all of which render against a CanvasDocument) working
  // exactly as before, without the user ever seeing a template gallery.
  React.useEffect(() => {
    if (selectedTemplateKey || templatesLoading || !effectiveImage) return;
    const fullBleed = templates.find((t) => t.key === 'FULL_BLEED');
    if (fullBleed) selectTemplate(fullBleed.key, fullBleed.canvas);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedTemplateKey, templatesLoading, templates, effectiveImage]);
