...
import {
  ImageOff, Loader2, Undo2, Redo2, RotateCcw, Save, Maximize, Columns2, ArrowRight,
} from 'lucide-react';
...
export const StudioCenterCanvas: React.FC = React.memo(() => {
  ...
  if (currentStep === 0) {
    ...
    // The canvas being rendered is now always the automatic, borderless
    ...
    body = canvas ? (
      ...
      <EmptyState
        ...
        hint="Preview & Export needs a real product photo to render against."
        action={
          <Button size="sm" variant="outline" className="gap-1.5" onClick={() => setCurrentStep(1)}>
            <span>Go to Upload Images</span>
            <ArrowRight className="w-3.5 h-3.5" />
